...
class ClickTool : ToolHandler {
    ...
    override suspend fun execute(args: JsonObject): ToolResult {
        return try {
            ...
            when {
                !nodeId.isNullOrBlank() -> clickByNodeId(service, nodeId)
                x != null && y != null -> clickByCoords(x, y)
                else -> ToolResult(
                    success = false,
                    toolName = TOOL_NAME,
                    error = ToolError(
            ...
            ToolResult(
                success = false,
                toolName = TOOL_NAME,
                error = ToolError(code = "CLICK_FAILED", message = "Failed to click: ${e.message}")
            )
        }
    }

    private fun clickByNodeId(service: AndroidAgentAccessibilityService, nodeId: String): ToolResult {
            ...
            if (clicked) {
                ...
                return ToolResult(
                    ...
                    result = buildJsonObject {
                        put("nodeId", nodeId)
                        put("text", targetNode.text?.toString() ?: "")
                        put("tappedX", (b.left + b.right) / 2)
                        put("tappedY", (b.top + b.bottom) / 2)
                        put("method", "accessibility_click")
                    }
                )
            }

            // Fallback: gesture tap on the node's bounds
            val b = Rect()
            targetNode.getBoundsInScreen(b)
            val cx = ((b.left + b.right) / 2f)
            val cy = ((b.top + b.bottom) / 2f)
            val gestureOk = GestureController.performTap(cx, cy)
            return if (gestureOk) {
                ToolResult(
                    success = true,
                    toolName = TOOL_NAME,
                ...
                ToolResult(
                    success = false,
                    toolName = TOOL_NAME,
                    error = ToolError(code = "CLICK_FAILED", message = "Both accessibility and gesture click failed")
                )
            }
        }
    ...
    private fun clickByCoords(x: Int, y: Int): ToolResult {
        val ok = GestureController.performTap(x.toFloat(), y.toFloat())
        return if (ok) {
            ToolResult(
                success = true,
                toolName = TOOL_NAME,
            ...
            ToolResult(
                success = false,
                toolName = TOOL_NAME,
                error = ToolError(code = "CLICK_FAILED", message = "Gesture tap failed at ($x, $y)")
            )
        }
    }

    /** Find a node by its hashed ID (format: "node_12345"). */
    ...
    companion object {
        ...
        fun definition(): AgentTool = AgentTool(
            name = TOOL_NAME,
            description = "Tap a UI element by node_id (preferred) or x,y coordinates. Uses accessibility click first, gesture tap as fallback.",
            inputSchema = buildJsonObject {
                put("type", "object")
                put("properties", buildJsonObject {
                    put("node_id", buildJsonObject {
                        put("type", "string")
                        put("description", "Node ID of the element to click")
                    })
                    put("x", buildJsonObject {
                        put("type", "integer")
                        put("description", "X coordinate (fallback)")
                    })
                    put("y", buildJsonObject {
                        put("type", "integer")
                        put("description", "Y coordinate (fallback)")
                    })
                })
            },
